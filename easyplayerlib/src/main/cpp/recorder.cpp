...
void EasyRecorder::write() {
    ...
    if (out == NULL) {
        if(event_listener != nullptr){
            event_listener(RECORDER_STATE_ERROR,ERROR_MALLOC_CONTEXT,"分配context出错");
        }
        return;
    }
    if (!(out->oformat->flags & AVFMT_NOFILE)){
        ...
        if (err < 0) {
            if(event_listener != nullptr){
                event_listener(RECORDER_STATE_ERROR,ERROR_OPEN_FILE,"无法打开指定存储位置文件");
            }
            avformat_free_context(out);
            out = NULL;
            return;
    ...
    if ( err < 0) {
        ...
        avformat_free_context(out);
        if(event_listener != nullptr){
            event_listener(RECORDER_STATE_ERROR,err,"打开视频编码器失败");
        }
        out = NULL;
        return;;
    }
    ...
    if ( err < 0) {
        ...
        avformat_free_context(out);
        if(event_listener != nullptr){
            event_listener(RECORDER_STATE_ERROR,err,"打开音频编码器失败");
        }
        out = NULL;
        return;
    }
    ...
    if(err < 0){
        if(event_listener != nullptr){
            event_listener(RECORDER_STATE_ERROR,ERROR_WRITE_HEADER,"写入视频头文件出错");
        }
        avio_close(out->pb);
        avformat_free_context(out);
        out = NULL;
    ...
    if(event_listener != nullptr){
        event_listener(RECORDER_STATE_READY,0,"准备好录制了");
    }
    ...
    int64_t video_calc_duration = (double) AV_TIME_BASE / av_q2d((AVRational) {frame_rate, 1});
    int64_t video_duration = (double) video_calc_duration / (double) (av_q2d(out->streams[video_stream]->time_base) * AV_TIME_BASE);
    ...
    if (out && !(out->oformat->flags & AVFMT_NOFILE))
        avio_close(out->pb);
    if(event_listener != nullptr){
        event_listener(RECORDER_STATE_COMPLETED,0,"录制完成");
    }
}
...
//            av_dict_set(&param, "preset", "superfast", 0);
    ...
    if (!codec_out) {
        ...
        return stream_index == AVMEDIA_TYPE_AUDIO ? ERROR_FIND_AUDIO_ENCODER:ERROR_FIND_VIDEO_ENCODER;
    }
    ...
    if (avcodec_open2(avctx_out, codec_out, &param) < 0) {
        ...
        return stream_index == AVMEDIA_TYPE_AUDIO ? ERROR_OPEN_AUDIO_ENCODER :ERROR_OPEN_VIDEO_ENCODER;
    }
    av_dict_free(&param);
